...
export default function Events() {
  ...
  const normalize = (raw) => {
    // raw can be: array, { events: [...] }, { data: { events: [...] } }, { data: [...] }, { results: [...] }, etc
    if (!raw) return [];
    if (Array.isArray(raw)) return raw;
    if (Array.isArray(raw.events)) return raw.events;
    if (raw.data && Array.isArray(raw.data.events)) return raw.data.events;
    if (raw.data && Array.isArray(raw.data)) return raw.data;
    if (Array.isArray(raw.results)) return raw.results;

    // fallback: take the first array-valued property
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      console.log("GET /api/events raw:", raw);
  ...
  // ADD THIS FUNCTION - Handler for register button
  const handleRegister = (eventId) => {
    console.log("Registering for event:", eventId);
    // You can navigate to a registration page or open a modal
    // For now, let's navigate to contact page or create a registration route
    navigate(`/events/register/${eventId}`);

    // OR if you want to show an alert for now:
          ...
          {events.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {events.map((event, index) => {
                const id = event._id || event.id || index;
                const title = event.title || event.name || "Untitled Event";
                const description = event.description || event.summary || "";
                const dateVal = event.date || event.startDate || event.datetime;
                const dateStr = dateVal
                  ? new Date(dateVal).toLocaleString()
                  : "Date not set";
                const shortDate = dateVal
